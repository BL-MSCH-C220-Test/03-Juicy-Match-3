...
func _ready():
	...
	$Tween.interpolate_property(self, "position", position, new_pos, 2.0, Tween.TRANS_QUART, Tween.EASE_IN_OUT)
	$Tween.start()
	$Tween.interpolate_property(self, "modulate:a", 1, 0.5, 2.0, Tween.TRANS_LINEAR, Tween.EASE_IN_OUT)
	$Tween.start()
